...
internal class UserPreferencesUpdateServiceImpl(
    ...
    private fun updateSyncCourses(cliOptions: CliOptions, prefs: UserPreferences): Pair<Collection<Course>, UserPreferences> {
        ...
        val coursesToSync = getCoursesToSync(cliOptions, prefs, coursesFromIlias)
        // update ids - some might not exist anymore
        return Pair(coursesToSync, prefs.copy(activeCourses = coursesToSync.map { it.id }.distinct()))
    }
    ...
    private fun updateFileSizeLimitFromCliOpts(prefs: UserPreferences, cliOptions: CliOptions): UserPreferences {
    ...
    /**
     * Prompts the user to select the positions of the [Course]s to sync. If the input is empty (default) all courses are selected.
     * Otherwise the space separated positions (1 based) are taken from the `allCourses` argument.
     *
     * @param allCourses
    ...
    private fun showAndSaveCourseSelection(allCourses: List<Course>): Collection<Course> {
        ...
        if (trimmedSelection.isNotBlank()) {
            val courseIndices = courseSelection.split("""\s+""".toRegex()).map { it.toLong() }.map { it - 1 }
